...
const actualizarEstado = ({ id, estado }) => {
	ClienteService.cambiarEstadoCliente(id, { estado: !estado })
		.then(() => {
			const cliente = clientes.value.findIndex((cliente) => cliente.id === id);
			clientes.value[cliente].estado = !estado;
		})
		.catch((error) => console.log(error))
}
...
<template>
	<div>
		...
		<p v-else class="text-center mt-10 font-bold">No hay clientes</p>
	</div>
</template>
